...
def register():
    ...
    if request.method == "POST":
        ...
        password = request.form.get("password")
        # Email is optional if not strictly required by DB
        ...
        # This part might not be reachable if methods are GET/POST only
        return error_page("Registration Error")
...
def newProject():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Insert the main project as a top-level work_item associated with
            # the project
...
def edit_project(project_id):
    ...
    if request.method == "GET":
        ...
        new_title = request.form.get("projectTitle")  # Get new title from form
        # new_description = request.form.get("projectDescription") # Add if you
        # expand the form
        ...
        try:
            ...
            # Assuming the main project work_item has parent_item_id IS NULL
            # and name matches project name initially
            cursor.execute("""UPDATE work_items
                             SET name = ?
                             WHERE project_id = ? AND parent_item_id IS NULL
...
def tasks(project_id):
    ...
    if project_details is None:
        ...
        # Check if the project exists at all to differentiate between "not
        # yours" and "not found".
        cursor.execute(
        ...
        if cursor.fetchone():
            return error_page(
                ...
                403)  # Project exists, but not for this user
        else:
            return error_page(
                "Project not found.",
                404)  # Project does not exist

    # Get name directly from project_details
    ...
    try:
        ...
        if len(deleted_task_ids) > 0:
            ...
            if clean_deleted_ids:  # Only proceed if there are valid IDs to delete
                ...
                print(
                    f"INFO: No valid numeric IDs found for deletion in {deleted_task_ids}")
            ...
            for task in task_list:
                ...
                # Check for existing tasks (numeric ID)
                    ...
                    try:
                        ...
                        print(
                            f"ERROR: Could not convert client_id '{client_id}' to int. Skipping update for this task.")
                        continue  # Continue to the next task without processing subtasks for this invalid one
                else:
                    print(
                        f"WARNING: Skipping task '{name}' with invalid client_id '{client_id}' (expected existing numeric ID or 'new-' prefix).")
                    continue  # Continue to the next task if ID is invalid

                # Recursively process subtasks ONLY if current_db_id was
        ...
        # SQL query to update top-level work_items names to match their project
        # name
        update_query = """
        UPDATE work_items
        SET 
